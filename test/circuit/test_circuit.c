...
void test_create_circuit_should_truncate_long_name(void) {
    const char* long_name = "This is a very long circuit name that exceeds the maximum length";
    Sector sectors[1] = {{0}}; // Dummy sector
    uint8_t num_sectors = 1;

    Circuit* result = create_circuit(long_name, sectors, num_sectors);

    TEST_ASSERT_NOT_NULL(result);
    TEST_ASSERT_EQUAL_STRING_LEN(long_name, result->name, sizeof(result->name) - 1);
...
void test_create_circuit_with_valid_inputs(void)
{
    ...
    Circuit* result = create_circuit(name, sectors, num_sectors);
